...
impl Challenger {
    ...
    pub async fn new(rpc_url: String, ws_url: String, private_key: String) -> Result<Self> {
        ...
        let service_manager_contract = TradeManager::new(service_manager_address, &pr);
    ...
    pub async fn start_challenger(&mut self) -> Result<()> {
        ...
        let responded_filter = Filter::new()
            .address(self.service_manager_address)
            .event_signature(TradeManager::TaskResponded::SIGNATURE_HASH)
            .from_block(BlockNumberOrTag::Latest);
        ...
        loop {
            tokio::select! {
                Some(log) = new_task_stream.next() => {
                    let decode = log.log_decode::<TradeManager::NewTaskCreated>().ok();
                    if let Some(decoded) = decode {
                        self.handle_task_creation(decoded);
                    }
                },
                Some(log) = responded_stream.next() => {
                    let decode = log.log_decode::<TradeManager::TaskResponded>().ok();
                    if let Some(decoded) = decode {
    ...
    fn handle_task_creation(&mut self, decoded: Log<TradeManager::NewTaskCreated>) {
    ...
    fn handle_task_response(&mut self, decoded: Log<TradeManager::TaskResponded>) {
    ...
    async fn slash_operator(&self, task: Task, task_index: u32) -> Result<()> {
        ...
        let swap_manager_contract = TradeManager::new(self.service_manager_address, &pr);
