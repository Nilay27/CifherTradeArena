...
let universalHookAddress: string | null = null;
try {
...
let testIntents: SwapIntent[] = [];

async function encryptAmount(amount: bigint): Promise<any> {
    try {
...
async function main() {
    ...
    console.log("UniversalHookABI functions:", UniversalHookABI.filter((x: any) => x.type === 'function').map((x: any) => x.name));
